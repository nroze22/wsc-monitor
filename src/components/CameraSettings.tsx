...
export default function CameraSettings() {
  ...
  const drawZones = () => {
    ...
    zones.forEach((zone) => {
      ...
      ctx.fillStyle = priorityColors[zone.monitoringPriority] || priorityColors.medium;
      ctx.beginPath();
      ctx.arc(zone.x + zone.width - 10, zone.y + 10, 5, 0, Math.PI * 2);
      ctx.fill();
    });
  };
      ...
      <div className="absolute top-4 right-4 flex items-center space-x-4">
        <div className="bg-gray-800 p-4 rounded-lg shadow-lg">
          <div className="flex items-center space-x-2 mb-4 text-gray-300">
            <HelpCircle size={16} />
            <span className="text-sm">Draw boxes to configure camera zones</span>
          </div>
